...
function Save-DatasetToBlob($datasetJson, $datasetName, $datasetFolderName, $accessToken) {
    ...
    Invoke-RestMethod -Method PUT -Uri $uri -Headers $headers -Body ([System.Text.Encoding]::UTF8.GetBytes($datasetJson))
}
...
try {
    ...
    $datasetListUrl = "https://management.azure.com/subscriptions/$($env:SUBSCRIPTION_ID)/resourceGroups/$($env:RESOURCE_GROUP)/providers/Microsoft.DataFactory/factories/$factoryName/datasets?api-version=2018-06-01"
   
    $datasets = Invoke-RestMethod -Uri $datasetListUrl -Headers @{ Authorization = "Bearer $managementToken" }
   
    foreach ($dataset in $datasets.value) {
        Write-Output "Processing dataset: $($dataset.name)"
        Write-Output "Saving dataset in the folder: $($dataset.properties.folder.name)"
        Save-DatasetToBlob ($dataset | ConvertTo-Json -Depth 10) $dataset.name $dataset.properties.folder.name $storageToken
    }
